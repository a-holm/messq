# Fixture: two transitions have swapped their events.
...
| T4 | INFLIGHT | `nak(token, delay?, reason?)` | fenced per S3.3 ∧ ¬(`max_deliver > 0` ∧ `attempts >= max_deliver`) | `state = READY`, `visible_at = now + (delay ?? backoff(attempts))`, `last_reason = reason` | `msg.timeout` | fence failure: as T3b; at the delivery bound: T5; `delay` outside `[0, 86400000]`: `errs.ErrBadRequest` |
...
| T8 | INFLIGHT | sweeper tick with `now > visible_at` | ¬(`max_deliver > 0` ∧ `attempts >= max_deliver`) | `state = READY`, `visible_at = now + backoff(attempts)`, `last_reason = "timeout"` | `msg.nak` (WARN) | at the delivery bound: T5 |

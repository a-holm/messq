...
// updateGolden regenerates testdata/schema_v4.golden from a fresh migration instead of
...
var updateGolden = flag.Bool("update-golden", false, "rewrite testdata/schema_v4.golden from a fresh migration")
...
// testdata/schema_v4.golden. This is the byte-level guarantee behind the frozen-artefact
...
// #7 added migration 0002 or #27 added 0004's ALTER).
func TestSchemaGolden(t *testing.T) {
	...
	const goldenPath = "testdata" + string(os.PathSeparator) + "schema_v4.golden"
	if *updateGolden {
...
func TestMigrateFreshCreatesSchemaV1(t *testing.T) {
	...
	if from != 0 || to != 4 {
		t.Fatalf("migrate fresh = (%d, %d), want (0, 4)", from, to)
	}
	if len(embeddedMigrations) != 4 {
		t.Fatalf("len(embeddedMigrations) = %d, want 4", len(embeddedMigrations))
	}
}
...
func TestMigrateFreshBookkeeping(t *testing.T) {
	...
	db := openTestDB(t, path)
	if got, want := metaValue(t, db, "schema_version"), "4"; got != want {
	...
	if got := queryInt(t, db, `PRAGMA user_version`); got != 4 {
		t.Fatalf("PRAGMA user_version = %d, want 4", got)
	}
}
...
func TestMigrateReopenIsNoop(t *testing.T) {
	...
	if from != 4 || to != 4 {
		t.Fatalf("migrate reopen = (%d, %d), want (4, 4)", from, to)
	}
	...
	if got := queryInt(t, db, `PRAGMA user_version`); got != 4 {
		t.Fatalf("PRAGMA user_version = %d after reopen, want 4", got)
	}
}
...
func TestMigrateRefusesSchemaTooNew(t *testing.T) {
	...
	if got := queryInt(t, db, `PRAGMA user_version`); got != 4 {
		t.Fatalf("PRAGMA user_version = %d after refusal, want the directory's own 4", got)
	}
}

...
func TestOpenFreshDirectoryLifecycle(t *testing.T) {
	...
	if report.SchemaFrom != 0 || report.SchemaTo != 4 {
		t.Errorf("report schema pair = (%d, %d), want (0, 4)", report.SchemaFrom, report.SchemaTo)
	}
	if !isULID(st.NodeID()) {
	...
	if got := st2.SchemaVersion(); got != 4 {
		t.Errorf("reopen SchemaVersion() = %d, want 4", got)
	}
...
func TestReadOnlyMode(t *testing.T) {
	...
	if got := ro.SchemaVersion(); got != 4 {
		t.Errorf("ReadOnly SchemaVersion() = %d, want 4", got)
	}

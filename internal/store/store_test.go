...
func TestOpenFreshDirectoryLifecycle(t *testing.T) {
	...
	if report.SchemaFrom != 0 || report.SchemaTo != 1 {
		t.Errorf("report schema pair = (%d, %d), want (0, 1)", report.SchemaFrom, report.SchemaTo)
	}
	if !isULID(st.NodeID()) {
	...
	if got := st2.SchemaVersion(); got != 1 {
		t.Errorf("reopen SchemaVersion() = %d, want 1", got)
	}
...
func TestReadOnlyMode(t *testing.T) {
	...
	if got := ro.SchemaVersion(); got != 1 {
		t.Errorf("ReadOnly SchemaVersion() = %d, want 1", got)
	}

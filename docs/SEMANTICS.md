...
| ID | Ambiguity | Resolution | Ratified in |
|---|---|---|---|
| C1 | PLAN.md section 4.2 declares `messages.id` "stable across DLQ/replay lineage" and, four lines later, a global `UNIQUE INDEX messages_id ON messages(id)`. Both cannot hold while the origin row still exists. | The dead-letter copy, a redrive and a replay each mint a **new** message id. `trace_id` is preserved, and the origin id travels in a provenance header and in the `detail` of the `msg.dead`, `dlq.redrive` and replay events. S4.4 has the lineage table. | ADR-0004 (D3) |
| C2 | PLAN.md section 11.4 lists "clock jumps (monotonic deadlines unaffected)" as a fault-injection case, which reads as a requirement that a wall-clock step leave ack deadlines alone. `internal/clock` stores durable deadlines as wall-clock milliseconds and states the opposite consequence. | Merged code wins. In-process waits are monotonic and a step does not disturb them. Durable deadlines are wall-clock: a forward step makes in-flight deliveries look overdue and the sweeper redelivers them with `reason=timeout`, which is correct at-least-once behaviour. S7 is normative. | ADR-0006 (D5) |
| C3 | PLAN.md section 5.1 guards both T1 and T2 on `max_ack_pending`, but a claim does not change `pending(c)`, so the T2 guard can never fire on its own. PLAN.md also writes T2's guard as `pending < max_ack_pending`, which would refuse the last claim once top-up has filled the pending set to the bound. | The bound is enforced where rows are created, at T1. T2 keeps the guard as an assertion, written `pending(c) <= max_ack_pending`; an implementation that finds it violated at claim has a bug in T1, not a flow-control decision to make. S5.3 defines `pending`. | ADR-0006 (D5) |
| C4 | PLAN.md section 5.1 writes T5's guard as `attempts >= max_deliver`, and the schema documents `max_deliver = 0` as unlimited. Read literally, `0 >= 0` dead-letters every first failure. | The guard is `max_deliver > 0 AND attempts >= max_deliver`. S6.3 works the boundary through for `max_deliver` in {0, 1, 5}. | ADR-0007 (D6) |
...
This table mirrors PLAN.md section 5.1 one row to one row, with one column PLAN.md does not carry: the outcome when the guard fails. The defining predicate of each state is not repeated per row; it is in S5.1, once. The conformance suite (#13) mirrors this table in the other direction: every ID below MUST appear in at least one test name.
...
Each invariant has a stable ID. The statements are those of PLAN.md section 5.2. "Checked by" names the mechanism; "First required at" names the issue whose merge must leave the invariant enforced. `messq verify` (#8), the rapid invariant hook (#13), test names, log messages and `docs/guarantees.md` (#35) all cite these IDs, so renaming one is a cross-repository rename and a breaking change under S16.

| ID | Statement | Predicate | Checked by | First required at |
|---|---|---|---|---|
| I1 | Every publish that returned 2xx under `full` durability is present after any crash, so no acknowledged publish is lost. | external three-valued ledger reconciliation: OK must exist, FAILED must not, UNKNOWN either | crash harness, `verify` | #8 |
| I2 | Every `(consumer, seq)` in `[first_seq, cursor_seq)` is in exactly one of resolved, READY or INFLIGHT. | S5.1's predicates are pairwise disjoint and cover the range | `verify`, rapid hook | #9 |
| I3 | An acked-and-committed `(consumer, seq)` is never redelivered except via an explicit seek, replay or redrive. | history predicate over `events`: no `msg.deliver` for a pair follows its `msg.ack` without an intervening `consumer.seek` or a new message id | rapid hook, golden log test | #10 |
| I4 | `attempts <= max_deliver` for every non-terminal row, and delivery stops at the bound, across restarts. | SQL predicate over `deliveries` joined to `consumers`, skipped where `max_deliver = 0` | `verify`, rapid hook | #11 |
...
| I7 | No stale-fenced ack, nak or term ever mutates a live row. | for every `msg.ack_stale` or `errs.ErrWrongGen` outcome, the row's `attempts`, `state` and `visible_at` are unchanged across the settle | unit tests, rapid hook | #10 |
| I8 | Each `(consumer, seq)` enters DEAD at most once, and each DEAD has exactly one dead-letter message when `dead_policy = dlq`. | count of `msg.dead` per pair is at most 1, and equals the count of `<s>.dlq` messages carrying that `Messq-Origin-Seq` and `Messq-Origin-Consumer` | `verify`, rapid hook | #12 |
| I9 | In a run with no faults where every delivery is acked within `ack_wait`, every message is delivered exactly once per consumer; any `attempts > 1` is preceded in the event log by a `msg.nak`, `msg.timeout` or `recovery.reclaimed` for that pair. | history predicate over `events`, cause set closed by S14.4 | rapid hook, golden log test | #13 |
| I10 | Folding the events table from the beginning reproduces the persisted state. | `verify --deep` replays the journal into a shadow state and diffs it against the tables | `verify --deep`, soak | #8 |
...
| delivery attempts | `consumers.max_deliver` | 5, 0 means unlimited | #11 |

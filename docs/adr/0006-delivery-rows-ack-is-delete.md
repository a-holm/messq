...
| A monotonic-anchored derived clock, so that a wall-clock step leaves ack deadlines untouched | Issue #4's design sketch proposed exactly this, and it prevents a redelivery burst after an NTP step. One reading of PLAN.md section 11.4's "clock jumps (monotonic deadlines unaffected)" asks for it too. | The merged `internal/clock` package stores durable deadlines as wall-clock milliseconds, because a deadline must survive a restart and be readable in SQL, and a monotonic reading is meaningless once the process that took it exits. Section 4.2 types `visible_at` that way, so the reading that would demand a derived clock contradicts the schema in the same plan. A step therefore does move deadlines, every resulting redelivery is a `msg.timeout` with a named cause, and the sweeper's per-tick cap bounds the burst. What section 11.4 does assert is kept exactly: the in-process waits are monotonic and a step does not disturb them. |
...
A wall-clock step is visible as a redelivery burst with `reason=timeout`. That is correct at-least-once behaviour and it is answerable, which is the property the product sells. Which readings are monotonic and which are not is recorded as clarification C2 in `SEMANTICS.md`.
